...
### Необязательно: лента новых анкет в общем чате

Бот может писать в тему супергруппы о каждой новой анкете — имя, кампус-отель и
кнопка «Открыть анкету».

```ini
TELEGRAM_FEED_CHAT_ID=@ex_tyrist   # ник группы; годится и числовой id
TELEGRAM_FEED_THREAD_ID=97         # номер темы
```

Номер темы — **средняя** часть ссылки на любое сообщение в ней:
`https://t.me/ex_tyrist/97/6697` → тема `97`. Без `TELEGRAM_FEED_THREAD_ID`
сообщения уйдут в «General».

Бот должен быть **в группе** и иметь право отправлять сообщения. Админом делать
не обязательно — но в закрытую тему пишут только админы.

Кнопка «Открыть анкету» ведёт в Main Mini App, поэтому не пропусти шаг 3
в разделе 8 — иначе ссылка выдаст «что-то пошло не так».

Переменные не заданы — лента просто выключена, всё остальное работает. Что
именно происходит, видно в логах при старте:

```bash
docker compose -p obshaga -f docker-compose.prod.yml logs backend | grep -i лента
```
...
2. `/setmenubutton` → выбери бота → отправь `https://disk.cu-politic.ru` → задай
   ...
   Telegram.
3. `/mybots` → бот → **Bot Settings** → **Configure Mini App** → **Enable Mini
   App** → отправь `https://disk.cu-politic.ru`

Третий шаг нужен для ссылок из общей ленты новых анкет: кнопка «Открыть анкету»
ведёт на `t.me/<бот>?startapp=p<id>`, а такой адрес открывает именно **Main Mini
App**. Кнопки меню из шага 2 для этого НЕ хватает — без шага 3 Telegram ответит
«что-то пошло не так, попробуйте позже».

Не хочешь включать Main Mini App — сделай `/newapp` с коротким именем и пропиши
его в `TELEGRAM_MINIAPP_SHORT_NAME`: ссылка станет `t.me/<бот>/<имя>?startapp=…`.
...
4. Подай заявку в комнату с другого аккаунта — жильцам прилетит уведомление.

---

## Почему сборка долгая и как ускорить

**Первая сборка на VPS честно занимает 10–20 минут** — это не баг. Скачиваются
базовые образы (python, node, nginx, postgres) и все пакеты с PyPI и npm.
Упирается всё в канал сервера, а не в процессор: пакеты мелкие, но у дешёвых
VPS аплинк до этих реестров часто медленный. Три сервиса собираются
параллельно и делят один канал — поэтому в логах у них похожее время.

**Дальше будет быстро.** Слои с зависимостями кэшируются: `requirements.txt` и
`package.json` копируются в образ до исходников, поэтому правки кода их не
инвалидируют. Плюс включены BuildKit cache mounts — кэш pip и npm переживает
пересборки (и лежит вне образа, так что образ не толстеет).

Что это значит на практике:

| Когда                                   | Сколько                                          |
| --------------------------------------- | ------------------------------------------------ |
| Первый запуск                           | 10–20 мин — качается всё, упирается в канал       |
| Правил только код (`git pull && up`)    | секунды: слой с зависимостями не пересобирается   |
| Менял `requirements.txt`/`package.json` | минуты: сеть нужна только для новых пакетов       |

Самый частый случай — правка кода — стоит секунд. Полные 10–20 минут ты платишь
один раз.

### Где происходит сборка

**Всё собирается на сервере.** В GitHub лежит только исходный код — Docker-образы
туда не коммитятся и «выгрузить сборку» в репозиторий нельзя:

```
GitHub  ──► только код (git push / git pull)
Сервер  ──► git pull → build (образы собираются здесь) → up -d
```

Собирать образы у себя на компьютере и везти на сервер — плохая идея: если у
тебя Mac на Apple Silicon, образы получатся `arm64`, а VPS почти всегда `amd64`
— они там не запустятся. Кросс-сборка через эмуляцию работает, но она в разы
медленнее, чем просто собрать на сервере.

Можно разделить долгий шаг и запуск, чтобы видеть прогресс (обе команды — на
сервере):

```bash
docker compose -p obshaga -f docker-compose.prod.yml --profile bot build   # долго
docker compose -p obshaga -f docker-compose.prod.yml --profile bot up -d   # секунды

# если PyPI тянется совсем медленно — можно взять зеркало:
# docker compose ... build --build-arg PIP_INDEX_URL=https://mirror.yandex.ru/mirrors/pypi/simple/
```

> Если однажды надоест ждать сборку на слабом VPS — правильный путь не «собрать
> локально», а собирать в GitHub Actions и класть образы в ghcr.io, чтобы сервер
> их только скачивал. Тогда деплой занимает ~минуту независимо от канала.

**Не отменяй сборку на середине** и не перезапускай её от нетерпения: прогресс
не потеряется, а повторный старт начнёт качать заново.

### Слабый сервер (1 ГБ RAM и меньше)

Две проблемы, обе решаемы.

**1. Сборки идут параллельно.** `docker compose build` собирает все сервисы
одновременно — на 0.6 ГБ они дерутся за память, и ядро убивает процесс. В логе
это выглядит как `Killed` или `exit code: 137`.

Решение — собирать по одному:

```bash
bash build-lowmem.sh
```

Скрипт проверяет swap, собирает сервисы поочерёдно (фронтенд последним — он
самый прожорливый) и чистит промежуточные слои между шагами. Или вручную:

```bash
C="docker compose -p obshaga -f docker-compose.prod.yml"
$C build backend && $C build bot && $C build frontend
$C --profile bot up -d
```

**2. Мало памяти в принципе.** Сначала сделай swap — на таком сервере он нужен
и для сборки, и для работы:

```bash
fallocate -l 2G /swapfile && chmod 600 /swapfile && mkswap /swapfile && swapon /swapfile
echo '/swapfile none swap sw 0 0' >> /etc/fstab
free -h   # должен появиться Swap: 2.0Gi
```

Swap на диске медленный, но он спасает от падения на пиках — а пик тут
короткий, только на этапе сборки фронтенда.

> Если сборка падает с OOM даже со swap — это тот случай, когда правильнее
> собирать не на сервере: GitHub Actions + ghcr.io, а VPS только скачивает
> готовые образы. Тогда 0.6 ГБ хватит с запасом, потому что сборки на нём не
> будет вовсе.
...
| Пачки 502 в разное время суток         | См. «502 пачками» ниже — это почти всегда встроенный DNS Docker или нехватка памяти, а не приложение |

---

## 502 пачками: DNS Docker и память

Симптом в логах Caddy — не отдельные ошибки, а именно **пачки** в узком окне:

```
dial tcp: lookup frontend on 127.0.0.11:53: server misbehaving
dial tcp: lookup frontend: i/o timeout
dial tcp 172.18.0.4:80: connect: connection refused
```

Ключ к диагнозу: если в то же окно Caddy не смог разрезолвить и внешний адрес —

```
lookup acme-v02.api.letsencrypt.org on 127.0.0.11:53: i/o timeout
```

— то ломается не сервис `frontend`, а встроенный DNS Docker (`127.0.0.11`) или
ресурсы хоста целиком. Приложение тут ни при чём, перезапускать его бесполезно.

Что смотреть:

```bash
# Не убивал ли ядро контейнеры за память
dmesg -T | grep -i -E "oom|killed process"
docker inspect obshaga-backend-1 --format '{{.State.OOMKilled}} {{.RestartCount}}'

# Перезапуски и падения по времени — сверь с окнами 502
docker events --since 24h --filter event=die --filter event=oom --filter event=restart

# Свободная память и своп
free -h
```

Частая причина на маленьком VPS — **нет свопа**. Пик памяти при сборке образов
или при обработке фото приводит к OOM-killer, тот убивает контейнер, а заодно
страдает `dockerd` с его DNS. Лечится добавлением своп-файла:
...
Со стороны приложения смягчено: Caddy теперь несколько секунд повторяет попытку
вместо мгновенного 502 (`lb_try_duration`), а у бэкенда есть healthcheck —
`docker ps` покажет `unhealthy` раньше, чем начнут жаловаться люди.
...
- **Авторизации по анкетам.** «Моя анкета» — это id в `localStorage` браузера.
  Для друзей по кампус-отелю хватает, но технически подкованный человек может дёрнуть
